...
def run(argv, label, env=None):
    ...
    rc = subprocess.call([PY, *argv], env=env)
...
#    The builtin _cinderx is linked into ./python, but the pure-Python `cinderx`
#    package (PythonLib) is NOT on the bare build tree's sys.path — CPython runs
#    PROFILE_TASK as `./python <task>` with no venv/.pth. Without it `import cinderx`
#    fails and CinderX records ZERO profile counts. build_static_cpython() passes the
#    PythonLib dir via CINDERX_PYTHONLIB; put it on PYTHONPATH for THIS subprocess
#    only (the -m test --pgo phase above stays path-pristine). The builtin _cinderx
#    still wins over any _cinderx.so in the PythonLib (BuiltinImporter precedes the
#    path finder), matching install_cinderx_pythonlib()'s "builtin + PythonLib" setup.
jit_env = os.environ.copy()
_pythonlib = os.environ.get("CINDERX_PYTHONLIB", "")
if _pythonlib:
    jit_env["PYTHONPATH"] = os.pathsep.join(
        [_pythonlib] + ([jit_env["PYTHONPATH"]] if jit_env.get("PYTHONPATH") else [])
    )
else:
    print("[pgo-train] WARNING: CINDERX_PYTHONLIB unset; cinderx may be unimportable "
          "during JIT training (zero CinderX profile coverage)", file=sys.stderr)
run([os.path.join(HERE, "cinderx_jit_workload.py")], "cinderx-jit", env=jit_env)
...
build_static_cpython() {
  ...
  # IMPORTANT: emit the whole _cinderx module definition on a SINGLE physical line
  # (no backslash line-continuation). makesetup joins '\'-continued lines before it
  # parses them (Modules/makesetup: `while echo $line | grep '\\$'`), so a continued
  # form builds a correct builtin _cinderx — BUT Tools/build/check_extension_modules.py
  # (run by `make`'s checksharedmods) parses Setup files line-by-line and does NOT
  # join continuations. It then treats every continuation line's first token
  # (cinderx/*.cpp, $(CINDERX_CFLAGS), -Wl,--start-group) as its own builtin module,
  # tries to import it, fails, and prints "[ERROR] ... failed to import" +
  # "Cannot mark builtin module ... as failed!" — 148 spurious errors plus a bogus
  # "148 failed on import" summary that reads as a build failure (and would HARD-fail
  # under PYTHONSTRICTEXTENSIONBUILD). One physical line makes both parsers agree:
  # makesetup sees the same joined line, check_extension_modules sees exactly one
  # module (_cinderx) and ignores the rest as sources.
  {
    ...
    echo "# NOTE: _cinderx is one long single line on purpose — see cinderx-benchmark.sh."
  ...
  if [ -n "$pgo_use_flag" ]; then
    ...
    # The CinderX+JIT half of PROFILE_TASK does `import cinderx`, but CPython runs
    # PROFILE_TASK as a bare `./python <task>` in the build tree with no venv/.pth,
    # so the pure-Python `cinderx` package (PythonLib) is not importable and CinderX
    # would train on ZERO data (its whole raison d'être here). Hand the PythonLib to
    # pgo_cinderx_train.py via CINDERX_PYTHONLIB; it puts it on PYTHONPATH for the JIT
    # workload subprocess only. Same PythonLib install_cinderx_pythonlib() uses.
    local cinderx_pythonlib="$SRC_CINDERX/cinderx/PythonLib"
    [ -d "$cinderx_pythonlib/cinderx" ] \
      || die "CinderX PythonLib not found at $cinderx_pythonlib (needed for PGO training import of cinderx)"
    ...
    # CINDERX_PYTHONLIB is set as a real env var (not a make-var) so GNU make
    # exports it to the PROFILE_TASK recipe subprocess, where pgo_cinderx_train.py
    # reads it. RUNSHARED (LD_LIBRARY_PATH) doesn't strip inherited env, so it flows
    # through ./python -> the train driver -> the JIT workload.
    if ( cd "$SRC_CPYTHON" && CINDERX_PYTHONLIB="$cinderx_pythonlib" \
           make profile-opt -j"$JOBS" PROFILE_TASK="$pgo_task" ) \
  ...
  if [ "$did_pgo_cycle" -eq 1 ]; then
    ...
    elif grep -q '\[jit-workload\] cinderx unavailable' "$LOGDIR/static_pgo_build.log" 2>/dev/null; then
      # This is the exact failure this task fixed: the JIT workload could not import
      # cinderx, so CinderX trained on ZERO profile data — the entire point of
      # source-listing it. Fail loudly instead of silently shipping a build with no
      # CinderX PGO coverage.
      die "PGO(static): CinderX JIT training could NOT import cinderx (see '[jit-workload] cinderx unavailable' in $LOGDIR/static_pgo_build.log) -> ZERO CinderX profile coverage. Check CINDERX_PYTHONLIB / PythonLib path."
    else
      die "PGO(static): CinderX JIT training banner ('[jit-workload] done') not found in $LOGDIR/static_pgo_build.log -> the JIT workload did not exercise CinderX (zero/partial CinderX profile coverage)."
    fi
...
install_cinderx() {
  ...
  ( cd "$SRC_CINDERX" \
    && env "${build_env[@]}" "$VPY" -m pip install . --no-build-isolation --force-reinstall \
  ...
  # foobar17 showed the dynamic _cinderx.so can be built WITHOUT LTO if the source
